...
class ClientTest extends AsyncFlatSpec with Matchers {
  ...
  "Client" should "be able to do GET requests" in { Client.get(uri("/posts/1")) map {_.isInstanceOf[JsValue]} }
        it should "be able to do POST requests" in { Client.post(uri("/posts/"), blank) map {_.isInstanceOf[JsValue]} }
        it should "be able to do PUT requests" in { Client.put(uri("/posts/1"), blank) map {_.isInstanceOf[JsValue]} }
        it should "be able to do PATCH requests" in { Client.patch(uri("/posts/1"), blank) map {_.isInstanceOf[JsValue]} }
        it should "be able to do DELETE requests" in { Client.delete(uri("/posts/1")) map {_.isInstanceOf[JsValue]} }
