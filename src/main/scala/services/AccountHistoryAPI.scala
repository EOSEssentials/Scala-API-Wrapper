...
object AccountHistoryAPI extends Settings {
  ...
  /***
    ...
    */
  def getTransaction(transactionId:String): Future[Option[GetTransactionWrapper]] =
    post(route("get_transaction"), Json.obj("transaction_id" -> transactionId)).map(_.map(_.as[GetTransactionWrapper]))
  ...
  /***
    ...
    */
  def getTransactions(accountName:String, skipSeq:Option[Int] = None, numSeq:Option[Int] = None):Future[Option[GetTransactionsWrapper]] =
    post(route("get_transactions"), Json.obj("account_name" -> accountName, "skip_seq" -> skipSeq, "num_seq" -> numSeq))
      .map(_.map(_.as[GetTransactionsWrapper]))

  /***
    * @param publicKey - The public get to get accounts for
    * @return - A list of account names
    */
  def getKeyAccounts(publicKey:String): Future[Option[List[String]]] =
    post(route("get_key_accounts"), Json.toJson("public_key" -> publicKey))
      .map(_.map(x => (x \ "account_names").as[List[String]]))

  /***
    * @param accountName - The account name to get controlled accounts for
    * @return - A list of account names
    */
  def getControlledAccounts(accountName:String): Future[Option[List[String]]] =
    post(route("get_controlled_accounts"), Json.obj("controlling_account" -> accountName))
      .map(_.map(x => (x \ "controlled_accounts").as[List[String]]))
